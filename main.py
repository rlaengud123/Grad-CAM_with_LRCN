...
def demo1(image_paths, topk, output_dir, cuda):
    ...
    model.eval()
    folder_count = 0
    image_folder_list = listdir_fullpath(image_paths[0])
    folder_list = os.listdir(image_paths[0])
    for k in image_folder_list:
        folder = folder_list[folder_count]
        folder_count = folder_count + 1
        for m in range(len(list_file)):
            count = 0
            image_list_temp = os.listdir(k)
            image_list = []
            for l in range(len(image_list_temp)):
                image_list.append(os.path.join(k, image_list_temp[l]))
            # Images
            images, raw_images = load_images(image_list)
        ...
        # =========================================================================
            ...
            if not os.path.isdir(output_dir + "/" + folder + '/' + target_layer):
                os.mkdir(output_dir + "/" + folder + '/' + target_layer)
            m_first = "00"
            if len(os.listdir('./results')) >= 10:
                m_first = "0"
            elif len(os.listdir('./results')) >= 100:
                m_first = ""
            m_name = m_first + str(len(os.listdir(os.path.join(output_dir, folder)))-1)
            for i in range(topk):
                ...
                for j in range(len(images)):
                    ...
                    count_name = count_first + str(count)
                    save_gradcam(
                        filename=osp.join(
                            output_dir,folder,target_layer,
                            "{}-{}-{}-gradcam-{}.png".format(
                                m_name, count_name, arch, target_layer
                            ),
                        ),
                        gcam=regions[j, 0],
...
@click.option("-o", "--output-dir", type=str, default="./results")
@click.option("--cuda/--cpu", default=True)
def demo3(image_paths, topk, stride, n_batches, output_dir, cuda):
    """
